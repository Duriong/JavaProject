...
public class MusicBox {//공유객체
	...
	public synchronized void playMusicA() {//1초 이하로 쉬면서 10번반복
		for(int i=0; i<10; i++) {
	...
	//팝송음악
	public synchronized void playMusicB() {//1초 이하로 쉬면서 10번반복
		for(int i=0; i<10; i++) {
	...
	//클래식음악
	//synchronized를 붙이지 않은 메소드는 다른 스레드들이 모니터링 락을 획득했어도, 그것과 상관없이 실행된다.
	public void playMusicC() {//1초 이하로 쉬면서 10번반복
		for(int i=0; i<10; i++) {
